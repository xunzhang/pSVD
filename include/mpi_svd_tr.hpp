...
#include "douban/mpi/linalg/mpi_bidiag.hpp"
	...
	namespace linalg {
		...
		template <class CAX, class CATX, class CU, class CS, class CV>
			...
			svd_tr(CAX && Ax, CATX && Atx, CU && U, CS && S, CV && V,
				...
				/* edited by wuhong */
				std::cout << "I am new here." << std::endl;
				int rank, nprocs;
				...
				std::cout << "I am old here0." << std::endl;
				int nev = std::max(U.dim1(), std::max(V.dim1(), S.size()));
				std::cout << "I am old here1." << std::endl;
				std::cout << "nev is " << nev << std::endl;
				std::cout << "min (m, n) " << std::min(m, n) << std::endl;
				if (nev <= 0) return 0;
				if (nev > std::min(m, n)) return -1;
				std::cout << "I am old here2." << std::endl;
				if (over_sample == 0) over_sample = nev;
				std::cout << "I am old here3." << std::endl;
				int k = nev + over_sample;
				std::cout << "I am old here4." << std::endl;
				k = std::min(k, std::min(m, n));
				std::cout << "I am old here5." << std::endl;
				std::vector<double> work(std::max(m, n) * k + 3 * k * k + 4 * k, 0);
				std::cout << "I am old here6." << std::endl;
				...
				std::cout << "I am old here.3" << std::endl;
				/* allocate limited here(on heap)! */
				mat_container<double, true> P(m, k);
				mat_container<double, true> Q(n, k + 1);
				P = 0;
				
				std::cout << "I am old here." << std::endl;
				/* edited by wuhong */
				if(rank == 0) {
					Q = 0;
