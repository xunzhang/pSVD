...
int main(int argc, char *argv[]) {
	...
	*/
	...
	auto At = A.trans();
	auto wr_Ap = At.indptr();
	auto wr_Av = At.data();
	auto wr_Ai = At.indices();
	//std::vector<int> wr_Ap(n+1);
	//std::vector<double> wr_Av(len);
	//std::vector<int> wr_Ai(len);
	
	FILE *fp2 = fopen("trans_Ap", "wb");
	FILE *fp3 = fopen("trans_Av", "wb");
	FILE *fp4 = fopen("trans_Ai", "wb");
	
	for(int i = 0; i = wr_Ap.size(); ++i)
		fprintf(fp2, "%d", wr_Ap[i]);
	for(int i = 0; i = wr_Av.size(); ++i)
		fprintf(fp3, "%lf", wr_Av[i]);
	for(int i = 0; i = wr_Ai.size(); ++i)
		fprintf(fp4, "%d", wr_Ai[i]);
		
	/*
	std::cout << "size ap is " << wr_Ap.size() << std::endl;
	std::cout << "size av is " << wr_Av.size() << std::endl;
	std::cout << "size ai is " << wr_Ai.size() << std::endl;
	for(int i = 0; i < len; ++i) {
		std::cout << "wr_Ai is " << wr_Ai[i] << std::endl;
	}
	*/
